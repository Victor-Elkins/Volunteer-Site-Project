...
export default function Dropdown({ selectedSkills, onSkillsChange }: DropdownProps) {
  ...
  const handleSelection = (skill: Skill) => {
    let updatedSelected = [...selected]
    if (updatedSelected.some(s => s.id === skill.id)) {
      updatedSelected = updatedSelected.filter(s => s.id !== skill.id)
    } else {
      updatedSelected.push(skill)
    }
    setSelected(updatedSelected)
    onSkillsChange(updatedSelected)
  }
